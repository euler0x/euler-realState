...
const INPUT_SCHEMA = {
  ...
  properties: {
    ...
    ubicacionPlanta: { type: ['string', 'null'], enum: ['frente', 'lateral', 'contrafrente', 'interno'] },
    antiguedadAnios: NULLABLE_NUM,
    estadoConservacion: { type: ['number', 'null'], enum: [1, 1.5, 2, 2.5, 3, 3.5, 4] },
    tieneCochera: { type: 'boolean' },
    tieneBaulera: { type: 'boolean' },
    amenities: { type: 'array', items: { type: 'string' } },
    categoriaConstructiva: {
      type: ['string', 'null'],
      enum: ['economica', 'estandar', 'buena', 'buena_servicios', 'premium'],
    },
